...
export const RequestHelp = () => {
  ...
  const [allRequests, allRequestsSet] = useState<HelpRequest_[]>([]);
  ...
  const showOrgSetup = !(user && user.organizations[0]);
  const [org, setOrg] = useState<OrganizationProfile_>();

  const fetchData = async () => {
    if (user && user.organizations[0]) 
    {
      let orgId = user.organizations[0].organziationId;
      const orgData = await getOrganizationProfile(BigInt(orgId)) as OrganizationProfile_;
      if (orgData === null) {
        navigate("/");
        return;
      }
      ...
      setOrg(orgData!);
      allRequestsSet([...orgData.requests.map(req => {
        req.person = orgData.persons.find(i => i.personId == req.personId)!;
        return req;
      })]);
    }
  ...
  //   if (showOrgSetup && user && user.organizations[0]) {
  //     const org = user.organizations[0];
  //     allRequestsSet([...org.requests.map(req => {
  //       req.person = org.persons.find(i => i.personId == req.personId)!;
  //       return req;
  //     })]);
  //   }
  // }, [user])
  ...
  const requestModalNext = (show: boolean, type: string, helperId: any) => {
    ...
    if (type == 'complete') {
      if (activeRequest) {
        //const helpers = [...activeRequest.helpers.map(i => { return { ...i, acceptComplete: i.userId == helperId } })];
        if (org) {
          fetchData();
          // setOrg({
          //   ...org, requests: [...org.requests.filter(i => i.requestId != activeRequest.requestId), {
  ...
  return allRequests && (
    <Layout title={`Help Requests`}>
      {showAddRequest && org && (<AddHelpRequestModal open={showAddRequest} setOpen={onRequestAdded} organization={org} />)}
      {showDetailsModal && (
          ...
          <div className="bg-white">
            <div className="max-w-7xl mx-auto text-center py-12 px-4 sm:px-6 lg:py-16 lg:px-8">
              <h2 className="text-3xl font-extrabold tracking-tight text-gray-900 sm:text-4xl">
                <span className="block">Ready to setup a non-profit?</span>
                <span className="block">Start creating help requests today.</span>
              </h2>
                      ...
                      <div className="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
                        <table className="min-w-full divide-y divide-gray-300">
                          ...
                          <tbody className="divide-y divide-gray-200 bg-white">
                            {allRequests.filter(i => !i.approvedHelper[0]?.acceptComplete).filter(filterRequests).map((req) => (
